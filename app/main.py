...
def active_trades(_: dict = Depends(require_authenticated)) -> dict:
    trades = get_operator_active_trades()
    if not trades:
        trades = get_active_trade_history()
        if trades:
