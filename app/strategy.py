...
def evaluate_breakout_strategy(
    ...
    completed_setup = _normalize_candles(_exclude_incomplete_candles(candles_5m))
    completed_trigger = _normalize_candles(_exclude_incomplete_candles(candles_1m))
    setup_candles = completed_setup or completed_trigger
    separated_timeframes = bool(completed_setup)

    minimum_setup_candles = max(EMA_BIAS_PERIOD, BREAKOUT_LEVEL_LOOKBACK)
    minimum_trigger_candles = max(RSI_PERIOD + 1, BREAKOUT_VOLUME_LOOKBACK + 1)
    if len(setup_candles) < minimum_setup_candles or len(completed_trigger) < minimum_trigger_candles:
        return _rejected_signal(symbol, "rejected", "missing_data", strategy_name=STRATEGY_BREAKOUT)

    latest = completed_trigger[-1]
    level_source = setup_candles if separated_timeframes else setup_candles[:-1]
    level_window = level_source[-BREAKOUT_LEVEL_LOOKBACK:]
    volume_window = completed_trigger[:-1][-BREAKOUT_VOLUME_LOOKBACK:]
    ...
    setup_closes = [candle.close for candle in setup_candles]
    trigger_closes = [candle.close for candle in completed_trigger]
    ema200 = _ema(setup_closes, EMA_BIAS_PERIOD)
    rsi_values = _rsi(trigger_closes, RSI_PERIOD)
    ...
    signal["setup_timeframe_used"] = separated_timeframes
    signal["setup_candle_count"] = len(setup_candles)
    signal["trigger_candle_count"] = len(completed_trigger)
    return signal
...
def evaluate_pure_smc_strategy(
    ...
    setup_candles = completed_setup or completed_trigger
    separated_timeframes = bool(completed_setup)
    timeframe_now = _normalize_now(now, completed_trigger or setup_candles)

    if len(setup_candles) < max(12, (SWING_CONFIRMATION_CANDLES * 2) + 5) or not completed_trigger:
        return _rejected_signal(symbol, "rejected", "missing_data", strategy_name=STRATEGY_PURE_SMC)

    setup = _find_pure_smc_setup(setup_candles)
    if setup is None:
        return _rejected_signal(
            symbol,
            "rejected",
            _last_pure_smc_reason(setup_candles),
            strategy_name=STRATEGY_PURE_SMC,
        )

    direction = str(setup["direction"])
    break_index = int(setup["break_index"])
    break_candle = setup_candles[break_index]
    latest_candle = completed_trigger[-1]
    order_block = _find_pure_smc_order_block(setup_candles, break_index, direction)
    if order_block is None:
        return _rejected_signal(symbol, "rejected", "invalid_trade_levels", strategy_name=STRATEGY_PURE_SMC)

    fvg_zone = _find_pure_smc_fvg(setup_candles, break_index, direction)
    if fvg_zone is None:
    ...
    if separated_timeframes:
        post_break_trigger = [candle for candle in completed_trigger if candle.timestamp >= break_candle.timestamp]
    else:
        post_break_trigger = completed_trigger[break_index + 1 :]
    if not post_break_trigger:
        post_break_trigger = [latest_candle]

    invalidated = _setup_invalidated(post_break_trigger, invalidation_level, direction)
    if invalidated:
        return _rejected_signal(symbol, "rejected", "setup_invalidated", strategy_name=STRATEGY_PURE_SMC)

    candles_since_break = len(post_break_trigger)
    if candles_since_break >= PURE_SMC_EXPIRY_CANDLES and not _candle_inside_zone(
        latest_candle, mitigation_zone[0], mitigation_zone[1]
    ):
        return _rejected_signal(
            symbol,
            "expired",
    ...
    payload["setup_timeframe_used"] = separated_timeframes
    payload["setup_candle_count"] = len(setup_candles)
    payload["trigger_candle_count"] = len(completed_trigger)
    payload["setup_detected_at"] = break_candle.timestamp.isoformat()
    return payload
